...
const editMemberEducationEntrySchema = z.object({
...
export const editMemberSchema = z.object({
  ...
  medicalRegNo: z
    ...
    .regex(/^[0-9০-৯a-zA-Z-]+$/, "শুধুমাত্র সংখ্যা ও ইংরেজি/বাংলা বর্ণ (a-z, A-Z), এবং ড্যাশ (-) ব্যবহার করতে পারবেন")
  ...
  educationEntries: z
    .array(editMemberEducationEntrySchema)
  ...
  notes: z
    ...
    profileImage: z
    .instanceof(File, { message: "প্রোফাইল ছবি সঠিক নয়" })
    .refine((file) => file.size > 0, "প্রোফাইল ছবি সঠিক নয়")
    .refine((file) => file.size <= MAX_FILE_SIZE, "ছবির সাইজ ৫MB এর কম হতে হবে")
    .refine((file) => ACCEPTED_IMAGE_TYPES.includes(file.type), "শুধু JPG, PNG, বা WEBP ছবি গ্রহণযোগ্য")
    .optional(),
});


export const partialEditMemberSchema = editMemberSchema.partial();
export type EditMemberFormValues = z.input<typeof editMemberSchema>;
export type PartialEditMemberFormValues = z.input<typeof partialEditMemberSchema>;
export type EditMemberDto = z.input<typeof editMemberSchema>;
export type PartialEditMemberDto = z.input<typeof partialEditMemberSchema>;